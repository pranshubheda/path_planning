...
def theta_star(source, destination):
    ...
    while not queue.empty():
        ...
        for succesor in succesors: 
            ...
            if succesor not in node_dict:
                ...
                #check if the successor can be reached in straight line from the
                #parent of current node
                current_node_parent = current_node.parent
                #handle case of start node
                if current_node_parent is not None:
                    # straight line from parent of current node to successor in continuous space
                    step_cost_path_2 = int(euclidean_distance(current_node_parent.coordinates, succesor) * 10)
                    path_2_cost = current_node_parent.path_cost + step_cost_path_2
                    path_2_priority = path_2_cost + heuristic
                    if succesor not in closed and path_2_cost < succesor_node.path_cost:
                        #check if you can travel in straightline without hitting obstacles
                        x1, y1 = current_node_parent.coordinates
                        x2, y2 = succesor
                        line_coordinates = list(bresenham(x1, y1, x2, y2))
                        line_obstacles = [grid[coordinate[0]][coordinate[1]] == 1 for coordinate in line_coordinates]
                        if not any(line_obstacles):
                            #no obstacles should be present
                            succesor_node.parent = current_node_parent
                            succesor_node.path_cost = path_2_cost
                            succesor_node.priority = path_2_priority
...
if __name__ == "__main__":
    ...
    #apply the theta_star algorithm to find path
    source = (0,28)
    # destination = (4,3)
    destination = (16,10)
    path = theta_star(source, destination)
    print(path)
    visualize_path(path)
    for line in grid:
        print(line)